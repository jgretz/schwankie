# Task: Port mac-mini runner wrapper for tasks supervisor
ID: ed6c8661 | Branch: feat/port-mac-mini-runner-wrapper-for-tasks-supervisor | Type: feat
...
**Branch**: feat/port-mac-mini-runner-wrapper-for-tasks-supervisor

### Last Checkpoint
Review requested changes — transitioning back to executing.

### Recent Progress
- [12:57] Self-review complete: Grade A

### Git State
Last commit: "fix(deploy): correct typecheck error output and rollback logging"
Uncommitted changes: 1 file

### PR
https://github.com/jgretz/schwankie/pull/84
...
**[T2]** performUpdate in runner.ts does not roll back when applyUpdate fails mid-way. If git checkout -B succeeds but bun install or typecheck throws, the repo HEAD moves to the new SHA. The catch block logs 'Update failed, staying on current code' and returns without calling rollback(). On the next poll, checkForUpdates sees localSha===remoteSha and permanently stops detecting the stale state. Fix: add `await rollback(updateConfig, previousSha)` in the catch block before return, in runner.ts ~line 51.

**[T1]** updater.ts applyUpdate(): info('Checking out latest release') is stale copy from stashl; the branch is main not a release branch. Change to 'Checking out latest commit'.
...
```
 deploy/mac-mini/README.md      | 141 +++++++++++++++++++++++++++++++++++++++++
 deploy/mac-mini/config.json    |  12 ++++
 deploy/mac-mini/src/child.ts   | 104 ++++++++++++++++++++++++++++++
 deploy/mac-mini/src/health.ts  |  46 ++++++++++++++
 deploy/mac-mini/src/log.ts     |  20 ++++++
 deploy/mac-mini/src/runner.ts  | 132 ++++++++++++++++++++++++++++++++++++++
 deploy/mac-mini/src/updater.ts | 121 +++++++++++++++++++++++++++++++++++
...
## Plan
## Context
WS-2 of the absorb-stashl.ink initiative. Stashl's `deploy/mac-mini/` (at `/Users/joshgretz/Development/Gretz/stashl.ink/deploy/mac-mini/`) contains a production-grade supervisor that watches a git branch, pulls, installs, type-checks, spawns the tasks worker, polls health, restarts with backoff. Schwankie has `deploy/api/` and `deploy/www/` (Fly) but no mac-mini tasks runner. WS-1 exposes /health on 3002.

Schwankie has no `release` branch; the port uses `main` and documents how to change in config. Multi-instance: each runner injects a unique WORKER_ID per process; PG-Boss claim semantics handle work contention.

## Flow
1. Operator runs `bun run deploy/mac-mini/src/runner.ts` (optionally with WORKER_ID=foo).
2. runner.ts spawns the apps/tasks child process, injects WORKER_ID env, logs via log.ts.
3. runner.ts polls origin/main every `updateIntervalMs`; on new commit: runs updater.ts (git pull, bun install, typecheck) → stops child gracefully → starts new child.
4. runner.ts polls `http://localhost:3002/health` via health.ts; restarts child on failure after retry budget.
5. SIGTERM/SIGINT: drains child (`drainTimeoutMs`) then exits.

## Steps
1. Create `deploy/mac-mini/src/log.ts` — verbatim from stashl.
2. Create `deploy/mac-mini/src/health.ts` — verbatim.
3. Create `deploy/mac-mini/src/child.ts` — copy; confirm `spawn('bun', ['run', '--cwd', 'apps/tasks', 'start'])` matches schwankie's apps/tasks start script.
4. Create `deploy/mac-mini/src/updater.ts` — copy; change typecheck command to `Bun.$\`bun run typecheck\`.cwd(repoDir)` (root workspace typecheck). Keep `bun install`.
5. Create `deploy/mac-mini/src/runner.ts` — copy; extend `buildEnv()` with `WORKER_ID: process.env.WORKER_ID ?? \`worker-${process.pid}\``.
6. Create `deploy/mac-mini/config.json` — remote: origin, branch: main, updateIntervalMs: 120000, drainTimeoutMs: 30000, health.url: http://localhost:3002/health, retries: 6, retryIntervalMs: 5000, initialDelayMs: 5000.
7. Create `deploy/mac-mini/README.md` — adapt stashl README: Schwankie branding, `/var/log/schwankie/runner.log`, branch=main, Multi-Instance section (WORKER_ID=worker-a, PG-Boss claim semantics), Boot Persistence section (nohup, tmux, Login Item plist).
8. Smoke test (post-WS-1 deploy): boot runner, confirm /health 200, push no-op commit to main, observe pull/install/typecheck/restart cycle; second runner with WORKER_ID=worker-b coexists.

## Acceptance Criteria

### Truths
- `bun run deploy/mac-mini/src/runner.ts` spawns apps/tasks child, injects WORKER_ID, polls origin/main, restarts on new commits, respects SIGTERM with drain.
- Second concurrent runner with different WORKER_ID coexists (health port conflict documented — only one runner binds 3002).
- README documents boot persistence + multi-instance usage.

### Artifacts
- deploy/mac-mini/{config.json, README.md} and deploy/mac-mini/src/{runner.ts, child.ts, updater.ts, health.ts, log.ts}.

### Key Links
- runner.ts orchestrates child.ts + updater.ts + health.ts; log.ts used by all.
- config.json drives all intervals + URLs; updater.ts reads branch + install commands.

### Verification
- `bun run typecheck` passes.
- Manual: git push to main triggers pull+restart; killing the worker child triggers health-failure restart; SIGTERM to runner drains child in <30s.

## Open Questions
- Release branch: schwankie has only main/master. Defaulting to main. Create a release branch?
- Port 3002 contention for second runner instance: only one can bind. Should WS-1 make port env-configurable, or should second runner skip HTTP health?
- `/var/log/schwankie/` requires sudo to create; README documents one-time setup.
- Typecheck scope: root `bun run typecheck` covers whole workspace; if slow, add a scoped script later.
...
- At the start of execution, run `worksite call check_inbox --json '{"task_id":"ed6c8661"}'` to read any messages from the orchestrator.
- Periodically (every 10-15 minutes of active work), re-run the same command to check for new directives.
- Run `worksite call send_progress --json '{"task_id":"ed6c8661","content":"<brief status>"}'` after completing each major step.

## Flagging Attention

See `.claude/rules/worksite-worker.md` for full guidance. In short:

- **Any time you would stop or wait** → `worksite call update_task --json '{"id":"ed6c8661","attentionMessage":"<reason>"}'`
...
Early in execution, run `worksite call update_task --json '{"id":"ed6c8661","fileIntents":["path/to/file.ts",...]}'` listing the repo-relative paths of files you plan to modify (derived from the plan). Update as your understanding evolves. This lets the orchestrator detect conflicts with parallel tasks.
...
```
worksite call send_progress --json '{"task_id":"ed6c8661","content":"Self-review complete: Grade [X]"}'
```
...
```
git push -u origin feat/port-mac-mini-runner-wrapper-for-tasks-supervisor
```
...
```bash
PR_BODY=$(worksite call generate_pr_description --json '{"task_id":"ed6c8661"}' | jq -r .body)
```
...
gh pr create --title "Port mac-mini runner wrapper for tasks supervisor" --label "Working" --body "$PR_BODY"
```
...
- Debugging insights, patterns, project quirks → `worksite call update_task --json '{"id":"ed6c8661","learning":"..."}'` (max 3-4)
- Permanent project conventions discovered → `worksite call create_idea --json '{"prompt":"[task:ed6c8661] Add rule: ..."}'` (max 2)
...
Run `worksite call submit_for_review --json '{"task_id":"ed6c8661","pr_url":"https://github.com/jgretz/schwankie/pull/84"}'` — this transitions the task and signals the system. Your job is done after this call.
...
2. **(Exception)** Justify: re-run the call with a `scopeJustifications` field, e.g. `worksite call submit_for_review --stdin <<'JSON' … JSON` with `{"task_id":"ed6c8661","pr_url":"https://github.com/jgretz/schwankie/pull/84","scopeJustifications":{"<file>":"<reason>"}}`. The reason must explain why this change is required by THIS task's acceptance criteria, not why it's convenient. The reviewer evaluates justifications and can still flag them as T3 scope findings.
...
2. Run `worksite call update_task --json '{"id":"ed6c8661","learning":"..."}'` for each meaningful learning (max 3-4)
...
3. Run `worksite call create_idea --json '{"prompt":"[task:ed6c8661] ..."}'` for out-of-scope improvements (max 2)
4. Run `worksite call update_task --json '{"id":"ed6c8661","note":"Retrospective: <2-3 sentences on what went well and what to improve>"}'`
5. Run `worksite call update_task --json '{"id":"ed6c8661","status":"user_review","note":"Reflection complete — ready for user review"}'`
...
3. `git push --force-with-lease`
   3b. **Verify clean state**: run `git status` (must show "nothing to commit, working tree clean") and `git log origin/feat/port-mac-mini-runner-wrapper-for-tasks-supervisor..HEAD` (must show 0 commits). If not clean or unpushed commits remain, push again.
4. If rebase fails after 2 attempts, run `worksite call update_task --json '{"id":"ed6c8661","attentionMessage":"Merge conflicts I cannot resolve: <summary>"}'`
5. Run `worksite call update_task --json '{"id":"ed6c8661","status":"user_review","note":"Resolved merge conflicts — returning to user review"}'` — do NOT call `submit_for_review`; the reviewer already approved the core implementation
...
1. Check your inbox: `worksite call check_inbox --json '{"task_id":"ed6c8661"}'`
2. Run `/review` (no arguments — reviews your diff against main)
3. Fix any findings below grade A, re-run typecheck and tests
4. Run `worksite call send_progress --json '{"task_id":"ed6c8661","content":"Self-review complete: Grade [X]"}'`
...
1. Check your inbox: `worksite call check_inbox --json '{"task_id":"ed6c8661"}'` — the directive contains the reviewer's specific findings
   1.5. Capture what the reviewer caught: `worksite call update_task --json '{"id":"ed6c8661","learning":"Reviewer flagged [what] because [why] — [takeaway for future tasks]"}'`.
...
6. **Verify clean state** before considering your work done:
   ...
   - `git log origin/feat/port-mac-mini-runner-wrapper-for-tasks-supervisor..HEAD` — must show 0 commits (all pushed)
     If either check fails, stage/commit/push the remaining changes before proceeding.
7. When all fixes are pushed, run `worksite call submit_for_review --json '{"task_id":"ed6c8661","pr_url":"https://github.com/jgretz/schwankie/pull/84"}'` — this re-triggers the reviewer immediately.
8. If you disagree with a finding after one attempt: `worksite call update_task --json '{"id":"ed6c8661","attentionMessage":"Reviewer disagreement: <summary>"}'` — do NOT keep trying
...
1. Read the PR comments: `gh pr view https://github.com/jgretz/schwankie/pull/84 --json comments,reviews`
2. Fix the code for each piece of feedback, commit, and push
   2b. **Verify clean state**: run `git status` (must show "nothing to commit, working tree clean") and `git log origin/feat/port-mac-mini-runner-wrapper-for-tasks-supervisor..HEAD` (must show 0 commits). If not, stage/commit/push remaining changes.
3. When all feedback is addressed, run `worksite call submit_for_review --json '{"task_id":"ed6c8661","pr_url":"https://github.com/jgretz/schwankie/pull/84"}'` — this sends your fixes through automated review before returning to the user.
