...
export const triggerRefreshAllFeedsAction = createServerFn({method: 'POST'}).handler(async () => {
  await getClient();
  await requireAuth();
...
export const triggerRefreshEmailsAction = createServerFn({method: 'POST'}).handler(async () => {
  await getClient();
  await requireAuth();
