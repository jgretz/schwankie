...
Every worksite tool mentioned below (`recall`, `list_tasks`, `create_idea`, `decay_memories`, etc.) is invoked as a `worksite` CLI subcommand via Bash:

```
worksite call <tool_name> --json '<json body>'
```

Example: "call `recall` with `limit: 5`" → `worksite call recall --json '{"query":"...","limit":5}'`.
...
- Scope:
  ...
  - "graph" -> Phase 12 only (knowledge graph analysis)
  - "decay" -> Phase 13 only (memory decay health)
- Flags:
...
The tool output includes a `Covered?` column and a `Recommendation` column:
- **Covered? = Yes** → `Current — no action`
- **Covered? = No** → targeted extraction guidance including domain, suggested doc location,
  outcome correlation (% crashed/stalled), and worker attention reasons

Report:

| File                         | Reads (total) | Tasks | Covered? | Recommendation                                                              |
| ---------------------------- | ------------- | ----- | -------- | --------------------------------------------------------------------------- |
| packages/store/src/schema.ts | 12            | 7     | No       | Domain: store. 30% of tasks ended crashed/stalled. Extract to .claude/rules/store-schema.md. |
...
If `fix` flag is present: for each uncovered file with taskCount >= 5, call `fix_uncovered_files`
which creates ideas with targeted recommendation text (domain, suggested location, attention reasons)
...
## Phase 12: Knowledge Graph Analysis (data-driven)

Call `query_graph` on key source files to identify coupling patterns. Select 5–8 representative files from
the most-imported modules in `packages/store/src/`, `packages/engine/src/`, and `apps/mcp/src/`.

For each file, call `query_graph` with `depth: 2` to capture transitive dependencies.

Report a table with columns: File, Imports (count), Callers (count), Unused Exports, Coupling Signal

| File                              | Imports | Callers | Unused Exports | Coupling Signal |
| --------------------------------- | ------- | ------- | -------------- | --------------- |
| packages/store/src/schema.ts      | 12      | 18      | —              | ⚠ high         |
| packages/engine/src/executor.ts   | 8       | 5       | configureTask  | —               |
| apps/mcp/src/handlers/index.ts    | 15      | 22      | —              | ⚠ high         |

- **Imports**: number of modules this file imports (direct dependencies)
- **Callers**: number of modules that call/import this file
- **Unused Exports**: exported symbols with no callers (potential dead code; note caveat about dynamic imports and framework conventions)
- **Coupling Signal**:
  - `⚠ high` if callers > 10
  - `⚡ moderate` if 5–10 callers
  - `—` if < 5 callers

Identify "hub files" where imports + callers combined > 15 — these are candidates for splitting or modularization.

### Interpretation

- Files with high coupling are integration points; refactoring them affects many modules
- Unused exports are candidates for removal (after verifying they're not used dynamically)
- Hub files indicate potential architectural boundaries that could be formalized

### Fix Mode

If `fix` flag is present: for each hub file, call `create_idea` suggesting a refactor:

```
prompt: "[introspect:graph] {filePath}: Split hub file — {imports} imports + {callers} callers = high coupling"
impact: 3
```

---

## Phase 13: Memory Decay Health (data-driven)

Call `decay_memories` to run a non-destructive decay pass and get current metrics.
Call `recall` with a broad query (e.g., `"*"` or empty string) with `k: 20` to sample active memories.

From the recalled memories, analyze:

1. **Decay pass results**: count of memories decayed (importance reduced) and soft-deleted (expired)
2. **Importance distribution**: classify sampled memories into low (<0.3), medium (0.3–0.7), high (>0.7) bands
3. **At-risk memories**: memories with importance ≤ 0.3 that haven't been accessed in 60+ days
4. **Source type health**: breakdown by source_type with average importance per type
...
| Metric | Count |
| ------ | ----- |
| Memories decayed (importance reduced) | N |
| Memories soft-deleted (expired) | N |

### Importance Distribution (sampled)
...
| Source Type | Count | Avg Importance | At-Risk |
| ----------- | ----- | -------------- | ------- |
| learning | N | 0.X | M |
| review_summary | N | 0.X | M |
| plan_chunk | N | 0.X | M |
| note | N | 0.X | M |

### Grading

Report a single grade based on fleet decay health:

- **A**: No at-risk memories, balanced importance distribution (all bands > 10%)
- **B**: ≤3 at-risk memories, <70% low importance
- **C**: 4–8 at-risk memories or >50% low importance
- **D**: >8 at-risk memories or >70% low importance
- **F**: Decay pass deleted >5 memories (significant knowledge loss)

### Differentiation

Phase 3 (Memory Hygiene) checks for overlap with rules and runs consolidation.
Phase 13 measures *decay health*: are memories accessed frequently enough to survive? Is knowledge being retained or eroding?

### Fix Mode

If `fix` flag is present and at-risk count > 0: for each at-risk memory, call `create_idea`:

```
prompt: "[introspect:decay] At-risk memory {memoryId}: {contentSnippet} — needs review or deletion"
impact: 2
```
...
| Pressure   | healthy/elevated/critical        | N high-pressure tasks, M avg peak   |
| Graph      | coupled/clean/no data            | N hub files, M unused exports       |
| Decay      | healthy/eroding/critical         | N at-risk, M deleted this pass      |
...
- Phase 12 requires 5–8 query_graph calls (one per key file) — moderate context cost
- Phase 13 requires 1 decay_memories call + 1 recall call — cheap
...
- /introspect pressure -> context pressure analysis across task fleet
- /introspect graph -> knowledge graph coupling analysis
- /introspect graph fix -> graph analysis + create ideas for hub files
- /introspect decay -> memory decay health metrics
- /introspect decay fix -> decay analysis + create ideas for at-risk memories
